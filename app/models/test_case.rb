class TestCase
  ...
  class << self
    ...
    def query(opts)
      all_cases = all
      str_date_opt = opts[:revision].map{|d| Revision.reformat_date(d) } rescue []
      ...
      all_cases.select!{|n| str_date_opt.include? Revision.reformat_date(n['revision']) } unless opts[:revision].blank?
    ...
    def namespaces(ns, opts)
      str_date_opt = opts[:revision].map{|d| Revision.date_to_str(Revision.str_to_date(d)) } rescue []
      ...
      ns.select!{|n| str_date_opt.include? Revision.date_to_str(n[:revision]) } unless opts[:revision].blank?
